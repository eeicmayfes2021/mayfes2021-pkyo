...
class SceneSelect extends Phaser.Scene {
    ...
    create(){
        ...
        if(window.savenum>-1){
            var LoadButton=new RoundedButton(this, 100, 320+40*Math.floor(num / 2), 180, 30, 0xfffff00, "Load", "red", 15)
            LoadButton.button.on('pointerdown', function(){
                this.scene.start("game",{stage_num:window.savenum});
            }.bind(this));
        }else{
            var LoadButton=new RoundedButton(this, 100, 320+40*Math.floor(num / 2), 180, 30, 0xfffff00, "Load", "gray", 15)    
        }
        //いらないボタンなどを隠す(クソ実装)
